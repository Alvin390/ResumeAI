...
export default function App() {
  ...
  // Navigate to dashboard once authenticated if currently on auth pages
  useEffect(() => {
    if ((location.pathname === '/login' || location.pathname === '/register') && authed) {
  ...
  useEffect(() => {
    const onMsg = (e) => {
      try {
        ...
        if (access) try { localStorage.setItem('access', access) } catch {}
        if (refresh) try { localStorage.setItem('refresh', refresh) } catch {}
        window.dispatchEvent(new Event('auth-changed'))
        window.dispatchEvent(new Event('storage'))
        // Try to close popup from opener if we still have a handle
        try {
          if (window._oauthPopup && !window._oauthPopup.closed) {
            window._oauthPopup.close()
          }
        } catch {}
        try { window._oauthPopup = null } catch {}
        navigate('/dashboard', { replace: true })
      } catch {}
    }
